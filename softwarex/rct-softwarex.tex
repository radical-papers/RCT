...
\begin{figure}
        ...
        \qquad
        \subfloat[ ]{{\includegraphics[width=0.45\textwidth]{figures/entk-tickets-year.pdf}}}
        ...
        maintenance.\jhanote{Figure quality needs addressing}}\label{fig:tags}
\end{figure}
...
\jhanote{Illustrative examples should illustrate not science impact but }\mtnote{I think your comment was truncated but I considered in the context of our previous discussion. Better?}
...
machines~\cite{radical-pub-url}. RCT support such applications by abstracting
...
failure~\cite{merzky2022raptor}. RCT allowed to develop a docking application
that used 8836 compute nodes on Frontera with between 1 and 31 concurrent
pilots, depending on different platform resource management policies. The same
application was ported to Summit~\cite{luo2020pre}, executing a different
docking kernel with minimal reconfiguration and a new task description on 1000
nodes with 6000 GPUs.

% $~17.4\times10^6$ docks/h on Frontera~\cite{stanzione2020frontera} and
% $11\times10^6$ docks/h on Summit~\cite{luo2020pre}. On frontera, RCT allowed to
...
source development of RCT\@. Currently, RCT supports a dozen active science
projects across the USA and Europe. The size of projects varies from single PIs
to large international collaborations.

\jhanote{I would organize impact around the three classes used in illustrative examples}\mtnote{Better?}
...
allowing to support the computational requirements of diverse scientific domain
applications at unprecedented scale, efficiency and effectiveness. RCT enabled
the of applications for chemical~\cite{sampat2018parallel} and
...
(e.g., bag of tasks, ensemble, pipelines and workflows), offering
...
pandemic, polar science, seas level rise) to utilize RCT without needed any
tailoring.
...
Beyond the application layer, as seen in \S\ref{sec:examples} RCT has enabled
the development of frameworks which provide domain specific functionalities.
ExTASY and RepEx implement advanced sampling algorithms using biomolecular
...
functionally-equivalent middleware, but they also allow users to choose among a
...
Finally, RCT are also a test bed for engineering research, mostly focused on
