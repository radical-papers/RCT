...
focuses on High Performance Computing (HPC) resources, enabling the execution of
...
\amnote{what does 'making' mean here?}
...
that is run as a self-contained executable.
...
purple components are responsible for managing pilots and units while yellow
...
manage pilots (PilotManager) and units (UnitManager). PilotManager has
...
their execution, this module distributes these files to the
...
environment.  On HPC resources which provide node local storage, RP can also
stage to that storage to improve task I/O.  Data can be exclusive to CUs, or can
be shared between CUs.

\ldots\mtnote{Andre to write this paragraph about the
architectural specifics of data management in RP, mentioning Stager Output in
Client.}\amnote{Does the above work?}

Each component of each subsystem of RP has a dedicated queue feeding entities
into that component. Orange Queues in
Fig.~\ref{fig:archs}a are dedicated to pilots and unit entities, blue
Queues to the messages exchanged by communication components.
All Queues support bulk communication to obtain
performance at scale in a distributed systems. Further, the Queues
enable load balancing between concurrent components (concurrent components are
used when using multiple physical for performance optimization).

A special Queue instance is rendered as collection in a MongoDB database.  That
collection is used to communicate between the client and agent subsystems,
whilemaintaining the Queue semantics used for all other Queues in
Fig.~\ref{fig:archs}a.  Since the MongoDB entries are (other then the ZMQ
messages from the other Queue instances) persistemt, that database is also used
to store data for post mortem analysis.
...
diagram.}\amnote{Does the above work?}
...
instruction of a program. \amnote{what is an 'instruction of a program'?} This is important: EnTK enables concurrent and
...
in a standalone SoftwareX publication.}\amnote{agree}
...
have dependences both among tasks and ensembles.\amnote{ah, here is the
ensemble definition :-)} These dependences express a
...
and tasks, or changing the properties of those already defined. Further,
