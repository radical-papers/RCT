...
functions, methods, threads or processes. \amnote{Things that \I{"run as
self-contained executables"} are usually processes.}
...
% \amnote{This is likely confusing if the reader does not know what an
% 'ensemble' is?}\mtnote{better?}
...
executable program\amnote{"represents an executable program"?} with information about its environment and data
...
and workload management.\amnote{I would probably leave out the "RabbitMQ" box
and label - that the queues are implemented in RMQ is an implementation detail,
similar to ZMQ on RP?  It's also (rightly) not mentioned in the text.}
...
platforms.\amnote{That's planned, but not sure?}
